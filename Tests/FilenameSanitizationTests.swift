...
final class FilenameSanitizationTests: XCTestCase {
    ...
    func test_does_not_split_grapheme_cluster() {
        // 👨‍👩‍👧 (family emoji) is one user-perceived character built from
        // five scalars joined by ZWJs: 👨 (4 bytes) + ZWJ (3) + 👩 (4) +
        // ZWJ (3) + 👧 (4) = 18 UTF-8 bytes. A scalar-walking truncation
        // could stop mid-sequence and produce a half-formed cluster that
        // renders as tofu in Finder / Dropbox listings. Character-walking
        // refuses the whole cluster when the budget can't fit it.
        let s = "a👨‍👩‍👧b"
        // bytes: a(1) + cluster(18) + b(1) = 20
        let result = s.truncatedToUTF8Bytes(18)
        XCTAssertEqual(result, "a")
        XCTAssertEqual(result.utf8.count, 1)
    }
}
