    ...
    var body: some View {
        listView
            ...
            .modifier(LibraryDialogs(
                ...
                gateError: $gateError,
                onDisconnect: {
                    // Production-only path; sync.dropbox is always non-nil
                    // when Library is reachable in a non-test build.
                    // Reset the seed flag synchronously — a "start over"
                    // disconnect should re-seed the sample if the next
                    // account's templates folder is empty. Without this, a
                    // manager who clears their templates folder and
                    // reconnects sees an empty library forever.
                    store.resetSeedFlag()
                    // unauthorize is async (v1.1) — it awaits the server-
                    // side token revoke before tearing down the local
                    // client so the next OAuth attempt doesn't race the
                    // in-flight revoke on SwiftyDropbox's shared
                    // NetworkSession (which surfaced as the "Couldn't open
                    // the Dropbox sign-in page" error on reconnect).
                    Task { await sync.dropbox?.unauthorize() }
                },
