...
struct FormRenderer {
    ...
    func render(
        ...
        // Title is the template name only. The signer's name lives in the
        // filename and in the rendered body — two intentional surfaces.
        // Embedding it a third time in PDF metadata adds no functional value
        // and creates a side-channel through OS-level Spotlight / search
        // indexers (which read kCGPDFContextTitle and surface the signer
        // name from indexed Dropbox folders even when the manager only
        // intends filename-pattern lookups).
        format.documentInfo = [
            kCGPDFContextTitle as String: template.name,
        ...
        //
        // autoreleasepool: the render path produces a 3x-oversampled signature
        // bitmap (~1.7MB for an Apple-Pencil-sized stroke), several scratch
        // path/text temporaries, and a logo `UIImage.draw` blit. Without an
        // explicit pool, these autoreleased objects hang on the surrounding
        // SwiftUI run-loop drain — so the peak survives through the view
        // update tick. The pool bounds it to one render.
        var data = Data()
        autoreleasepool {
