...
final class TemplateStore {
    ...
    /// Maps each in-memory template to its provider-opaque identifier (a
    /// Dropbox path today; a Drive fileId or iCloud URL later). Populated
    /// by `refresh()` from `TemplateRef.identifier` and by `save()` from
    /// the `saveTemplate` return value. Read by `delete()` to round-trip
    /// the exact identifier the provider issued, instead of synthesizing
    /// one from a folder constant + filename — which only worked for
    /// Dropbox by accident (case-insensitive path matching).
    private var templateIdentifiers: [UUID: String] = [:]

    /// Tracks the currently-running refresh task, if any. New `refresh()`
    /// calls await this task instead of spawning a parallel one (overlapping
    /// refreshes used to commit out-of-order, so a slow refresh could
    /// wholesale-replace a fresh list with stale data). `save()` and
    /// `delete()` await this task before mutating so a refresh whose
    /// list+download work resolves after a save can't drop the just-saved
    /// template's identifier (audit ME-07/ME-08).
    ...
    private let demoChecker: () -> Bool
    private let defaults: UserDefaults

    /// `provider` is the CRUD handle (a real `DropboxSyncProvider` in
    /// production; a fake in tests). `isDemo` is a closure rather than a
    /// captured `SyncCoordinator` so tests can swap in `{ false }` without
    /// wiring up the full coordinator + provider graph.
    init(
        provider: any SyncProvider,
        isDemo: @escaping () -> Bool = { false },
        defaults: UserDefaults = .standard
    ) {
        self.provider = provider
        self.demoChecker = isDemo
        self.defaults = defaults
        self.templates = []
    }

    /// Convenience for production wiring — captures the coordinator's
    /// `isDemo` getter so the store reacts to demo enter / exit without
    /// each consumer threading the bit through.
    convenience init(sync: SyncCoordinator, defaults: UserDefaults = .standard) {
        self.init(
            provider: sync.provider,
            isDemo: { [sync] in sync.isDemo },
            defaults: defaults
        )
    }

    /// Pulls the latest template set from the configured provider, decodes
    /// each, and rebuilds `templates` sorted by name. A failure leaves the
    /// list intact at its last good state and surfaces a message via
    /// `loadState`.
    ///
    /// First-run seeding: when the user has zero templates AND we haven't
    /// seeded before on this iPad, write the sample template to the provider
    /// so the library has something to show on first launch. The
    /// `hasSeededSample` flag is local to the device, so a second iPad
    /// joining an existing account won't re-seed.
    ///
    /// Concurrency: overlapping callers coalesce on `inFlightRefresh`.
    /// `save()` / `delete()` also await this task before mutating so a
    /// refresh that lands after a save can't wholesale-replace
    /// `templates`/`templateIdentifiers` and drop the just-saved entry.
    func refresh() async {
    ...
    private func performRefresh() async {
        // Demo session never touches the provider. First refresh seeds the
        ...
        var skipped = 0
        do {
            let refs = try await provider.listTemplates()
            ...
            let outcomes = await withTaskGroup(
                of: (TemplateRef, Result<FormTemplate, Error>).self,
                returning: [(TemplateRef, Result<FormTemplate, Error>)].self
            ) { group in
                for ref in refs {
                    group.addTask {
                        do {
                            let data = try await providerRef.downloadTemplate(at: ref.identifier)
                ...
                var out: [(TemplateRef, Result<FormTemplate, Error>)] = []
            ...
            for (ref, result) in outcomes {
                ...
                case .success(let decoded):
                    ...
                    identifiers[decoded.id] = ref.identifier
            ...
            if refs.isEmpty && !defaults.bool(forKey: Self.hasSeededSampleKey) {
                ...
                do {
                    ...
                    let identifier = try await provider.saveTemplate(data, filename: Self.filename(for: sample))
                    defaults.set(true, forKey: Self.hasSeededSampleKey)
                    loaded.append(sample)
                    identifiers[sample.id] = identifier
                } catch {
    ...
    func save(_ template: FormTemplate) async throws {
        ...
        // Let any in-flight refresh complete its atomic-swap before we
        // mutate. Without this gate, a refresh whose listTemplates+download
        // round-trip finishes after our local mutation can wholesale-replace
        // `templates`/`templateIdentifiers` with the pre-save state and drop
        // our new entry. (audit ME-07/ME-08)
        await inFlightRefresh?.value
        // Demo session: in-memory write only, no provider round-trip.
        if demoChecker() {
        ...
        let identifier = try await provider.saveTemplate(data, filename: Self.filename(for: template))
        var updated = templates.filter { $0.id != template.id }
        updated.append(template)
        templates = updated.sorted {
            $0.name.localizedCaseInsensitiveCompare($1.name) == .orderedAscending
        }
        templateIdentifiers[template.id] = identifier
    }

    /// Deletes a template from the provider and the local list. Non-editable
    ...
    func delete(_ template: FormTemplate) async throws {
        ...
        // Mirror save()'s gate: a refresh that lands after delete's local
        // mutation would resurrect the just-deleted template by repopulating
        // `templates` from the provider's pre-delete listing. Await the
        // in-flight refresh first.
        await inFlightRefresh?.value
        // Demo session: in-memory delete only, no provider round-trip.
        if demoChecker() {
            templates.removeAll { $0.id == template.id }
            templateIdentifiers.removeValue(forKey: template.id)
            return
        }
        // Every template visible in the library got here via either
        // `refresh()` or `save()`, both of which populate
        // `templateIdentifiers`. A miss here would indicate a code bug
        // (mutation outside this class) — surface it rather than papering
        // over with a synthesized provider-specific path.
        guard let identifier = templateIdentifiers[template.id] else {
            throw StoreError.identifierMissing
        }
    ...
    enum StoreError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .identifierMissing: return "Couldn't locate the template's saved location. Pull to refresh and try again."
            }
        }
    }
