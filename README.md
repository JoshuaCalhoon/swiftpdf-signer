...
A native iOS app for filling and signing PDF forms on iPad and iPhone, then uploading the signed copies to your own cloud storage. The 1.0 release ships with Dropbox; additional providers are planned post-1.0.

Hand a customer the iPad, they sign with finger or Pencil, the app flattens to PDF, names it deterministically (`{FormName} - {SignerName} [{Date}].pdf`), and uploads to a dedicated app folder on your connected provider.
...
- A `SyncProvider` protocol for cloud uploads; v1 ships a Dropbox conformer using [SwiftyDropbox](https://github.com/dropbox/SwiftyDropbox) for OAuth (PKCE) + file upload to an app folder
...
- A Dropbox app registered at <https://www.dropbox.com/developers/apps> (scope: **App folder**; permissions: `files.content.read` + `files.content.write`). Other providers are planned for future releases.
